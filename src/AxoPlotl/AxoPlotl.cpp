...
void MeshViewer::run()
{
    ...
    {
        AxPl::TetrahedralMesh tetMesh;
        readMesh(filename, tetMesh, AxPl::FileFormat::OVMB);
...
void MeshViewer::init()
{
    ...
    glfwSetFramebufferSizeCallback(window, AxPl::framebuffer_size_callback);
    glfwSwapInterval(1); // <- Without this as many frames as possible are rendered and the GPU suffers

    //glfwSetInputMode(window, GLFW_CURSOR, GLFW_CURSOR_DISABLED);
    glfwSetCursorPosCallback(window, AxPl::mouse_callback);
    glfwSetScrollCallback(window, AxPl::scroll_callback);
    ...
    // ImGui
    //------------------------------------------
    AxPl::ImGuiRenderer::init(window);
...
void MeshViewer::render()
{
    ...
    // Picking
    if (AxPl::MouseHandler::LEFT_JUST_PRESSED)
    {
        ...
        int x = AxPl::MouseHandler::POSITION[0] * xscale / framebuffer_width * pickingTexture.getWidth();
        int y = AxPl::MouseHandler::POSITION[1] * yscale / framebuffer_height * pickingTexture.getHeight();
    ...
    AxPl::ImGuiRenderer::newFrame();

    // Render
    renderer.render(matrices);

    // ImGui - define and render
    AxPl::ImGuiRenderer::render(*this, renderer.settings);
    ...
    AxPl::MouseHandler::update(window);
    AxPl::Time::update();
