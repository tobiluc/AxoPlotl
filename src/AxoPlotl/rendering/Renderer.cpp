...
// void Renderer::addMesh(PolyhedralMesh &mesh, BatchIndices& loc)
// {
//     // Add Points
//     if (mesh.n_vertices() > 0)
//     {
//         uint points_batch_index = findEmptyPointsBatch();
//         points[points_batch_index]->regenerateBuffers(mesh.n_vertices());
//         for (int i = 0 ;i < mesh.n_vertices(); ++i) {
//             points[points_batch_index]->add(Point(mesh.vertex(OVM::VH(i)), Color::LIGHTGRAY));
//         }
//         loc.emplace_back(GL_POINTS, points_batch_index);
//     }
...
//             n_tris += mesh.valence(OVM::FH(i))-2;
//         }
...
//         for (int i = 0; i < mesh.n_faces(); ++i) {
//             const auto& vhs = mesh.get_halfface_vertices(OVM::FH(i).halfface_handle(0));

//                 for (int j = 1; j < vhs.size()-1; ++j) {
//                     OVM::VH v0 = vhs[0];
//                     OVM::VH v1 = vhs[j];
//                     OVM::VH v2 = vhs[j+1];

//                     tris.emplace_back(
//                         toVec3<Vec3f>(mesh.vertex(v0)), toVec3<Vec3f>(mesh.vertex(v1)), toVec3<Vec3f>(mesh.vertex(v2)),
//                         Color::WHITE
//                         );
//                 }
//         }
